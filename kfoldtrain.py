...
train_data = read_dataset(config.train_path, config.vocab_path) #TODO: 注意修改路径
...
def init_model():
    ...
    if(config.use_attack):
        attacker = FGM(model)
    return model, attacker, scheduler, optim

def init_loader(fold):
        dev_start = int(fold * 0.2 * num_lines)
        dev_end = int((fold+1) * 0.2 * num_lines)
        dev_loader = DataLoader(TextDataset(train_data[dev_start:dev_end]),batch_size=config.batch_size, shuffle=False)
        train_loader = DataLoader(TextDataset(train_data[:dev_start] + train_data[dev_end:]), batch_size=config.batch_size, shuffle=True)
        return train_loader, dev_loader

def init_loader_from_path(fold):
    train_lines = read_dataset(f'./data/traindata_odd_fold{fold}.tsv', config.vocab_path)
    dev_lines = read_dataset(f'./data/devdata_odd_fold{fold}.tsv', config.vocab_path)
    dev_loader = DataLoader(TextDataset(dev_lines),batch_size=config.batch_size, shuffle=False)
    train_loader = DataLoader(TextDataset(train_lines), batch_size=config.batch_size, shuffle=True)
    return train_loader, dev_loader
...
def train():
    losses_list = []
    for fold in range(5): # 五折交叉验证
        log.info(f'==============fold: {fold} ================')
        train_loader, dev_loader = init_loader_from_path(fold) # 初始化每一折的loader
        model, attacker, scheduler, optim = init_model() # 初始化每一折的model
        best_auc = 0.5
        for epoch in range(num_epochs):
            ...
            for text,label,token_type in pbar:
                ...
                if step % 100 == 0:
                    log.info('----start eval ----')
                    cur_auc = evaluate(model, dev_loader)
                    if(cur_auc > best_auc and cur_auc > 0.97):
                        best_auc = cur_auc
                        torch.save(model.module.state_dict(),f'model/nezhaoddngram_fold{fold}_{best_auc:.5f}.pth', _use_new_zipfile_serialization=False)
                    model.train()
        ...
        log.info(f'=============fold:{fold}, best_auc:{best_auc}==============')
